...
async function renderBlogPost(slug: string): Promise<RenderResult> {
  ...
  // Keep verification markers useful and readable in prerendered HTML.
  articleBodyHtml = articleBodyHtml.replace(
    /<span[^>]*class="[^"]*verify-tag[^"]*"[^>]*title="([^"]*)"[^>]*>\[[^<]*\]<\/span>/g,
    (_m, title) =>
      `<span class="verify-tag" title="${esc(title)}" style="display:inline-block;font-size:11px;line-height:1;vertical-align:super;padding:1px 5px;margin-left:2px;background:#f1f5f9;color:#0f172a;border:1px solid #e2e8f0;border-radius:4px;cursor:help;text-decoration:none;">✓ Verified</span>`,
  );
  articleBodyHtml = articleBodyHtml.replace(
  ...
  articleBodyHtml = articleBodyHtml.replace(/\[(CHART|DIAGRAM|IMAGE|SCREENSHOT|STEP-BY-STEP)(?:\s+NEEDED)?:[^\]]*\]/gi, "");
  articleBodyHtml = articleBodyHtml.replace(/<figcaption>\s*Image:\s*Generated by Google Imagen\s*<\/figcaption>/gi, "");
  articleBodyHtml = articleBodyHtml.replace(/<figcaption>\s*Generated by Google Imagen\s*<\/figcaption>/gi, "");
  articleBodyHtml = articleBodyHtml.replace(
  ...
  articleBodyHtml = articleBodyHtml.replace(
    ...
    (_m, h2, pStart, classes, pEnd) =>
      /\bsection-summary\b/.test(classes)
        ? `${h2}${pStart}${classes}${pEnd}`
        : `${h2}${pStart}${classes} section-summary${pEnd}`,
  );
