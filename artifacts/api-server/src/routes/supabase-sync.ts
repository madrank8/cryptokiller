...
router.post("/sync/supabase", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (!brands || brands.length === 0) {
      res.json({ message: "No brands found", syncedBrands: 0, syncedReviews: 0 });
      return;
    }
    ...
    if (reviews && reviews.length > 0) {
      ...
      for (const supaReview of reviews as SupaReview[]) {
        ...
        syncedReviews++;
        log.info(`Synced review: ${supaReview.slug}`);
      }
    }

    res.json({ message: "Sync complete", syncedBrands, syncedReviews });
  } catch (err) {
