...
- WhatsApp: `src/routes/whatsapp.ts` — Twilio webhook at `POST /api/whatsapp` with platform check, report, alerts, and menu flows. Requires `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_WHATSAPP_NUMBER` env vars.
...
- **Analytics**: Google Analytics 4 (`G-E529ZZ71JJ`) via `src/lib/analytics.ts` — gtag.js injected programmatically (no inline snippet, CSP-compatible), production-only, SPA page_views fired on wouter route changes by `AnalyticsTracker` in `App.tsx` (automatic page_view disabled to avoid double-count). CSP `script-src` allows `www.googletagmanager.com`.
